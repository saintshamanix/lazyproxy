...
- Имена входящих: флаг страны и протокол. На чистой установке создаётся только `User1` на REALITY. Остальные входящие создаются без клиентов; добавляйте или привязывайте пользователей в панели.
...
Укажите два разных DNS-имени: можно использовать домен и поддомен либо два поддомена.

| Параметр | Пример | Назначение |
|---|---|---|
| `WEB_DOMAIN` / `--domain` | `vpn.example.com` | Сайт, панель, подписки, WS, XHTTP, gRPC, Hysteria2 и адрес AWG |
| `REALITY_DOMAIN` / `--reality-domain` | `reality.example.com` | SNI REALITY и TLS fallback |

Создайте **A-запись для каждого имени, направленную прямо на IPv4 VPS**.
Проксирование CDN должно быть выключено (например, DNS only в Cloudflare).
Этот IPv4-only адаптер отклоняет ответы AAAA/CNAME; используйте прямые A-записи.
TCP/80 нужен для Let's Encrypt HTTP-01 и продления. Certbot получает один
сертификат на оба имени. Указывайте только имена, без `https://`, портов и paths.
Для интернационализированных доменов используйте ASCII/Punycode.
...
`--config /root/lazyproxy.env`. Значения файла переопределяют соответствующие
параметры CLI. Имена задаются только парой. Если на чистом VPS оба параметра
отсутствуют, используются прежние автодомены.

Повторный запуск без параметров домена сохраняет записанные имена, включая
собственные. Смена IP или любого домена существующей установки намеренно
отклоняется: сертификаты, ссылки и профили клиентов требуют отдельной миграции.
`--update-only` использует сохранённые домены и не принимает параметры домена.

## Опциональный режим публичный IP + TLS

На **чистом VPS** добавьте `--ip-tls` к команде установки (или `IP_TLS=yes`
в конфигурации root). По умолчанию сохраняется установка с автодоменами.
Режим нельзя совмещать с `--domain` / `--reality-domain`.

- Сайт, панель, ссылки подписки и TLS-подключения используют публичный IPv4.
- REALITY сохраняет `IP-с-дефисами.cdn-one.org` для SNI-диспетчера и локального TLS fallback.
  Его A-запись должна указывать на VPS: этот гибридный режим всё ещё зависит от DNS-имени.
- Один сертификат содержит IP SAN публичного адреса и DNS SAN REALITY. Клиенты
  проверяют сертификат IP; проверка сертификатов не отключается. Внутренний сервер
  подписок использует DNS SAN с проверяемым HTTPS.
- IP-сертификаты Let's Encrypt действуют **160 часов**, используется профиль `shortlived`.
  Certbot 5.8.0 устанавливается в `/opt/single443-certbot-5.8.0`, состояние ACME —
  в `/etc/single443/acme`, отдельно от системного Certbot. TCP/80 должен оставаться доступным.
- `single443-acme.timer` проверяет необходимость продления каждые шесть часов.
  После выпуска проверяется nginx, выполняется reload nginx и restart x-ui для
  сертификата Hysteria2. Неудачная перезагрузка повторяется при следующей успешной
  проверке продления, даже без перевыпуска. Диагностика требует минимум 24 часа до истечения.
- Повторный запуск без параметров сохраняет режим. Переключение существующей
  установки между доменным и IP-режимом отклоняется; существующие профили не меняются.
...
CI проверяет IP SAN с доверенным тестовым сертификатом и передачу XHTTP через
реальные шаблоны nginx. Выпуск публичного сертификата ACME и импорт в Shadowrocket/INCY
на реальном VPS требуют отдельного установочного теста; CI его не заменяет.

Источники: [IP-сертификаты Let's Encrypt](https://letsencrypt.org/2026/01/15/6day-and-ip-general-availability/),
[поддержка IP в Certbot](https://letsencrypt.org/2026/03/11/shorter-certs-certbot/).

Обновление существующей установки сохраняет её клиентов: это изменение не удаляет пользователей и подписки.
...
- Совместимость экспорта AmneziaWG `vpn://` с Shadowrocket не гарантируется.
  Для старых установок с предсозданным AWG-клиентом отдельный конфиг сохраняется в `/etc/single443/User6-AmneziaWG.conf`
