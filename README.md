...
* `/routing/incy.json` — приложенный пользователем профиль **Shamanix RU Direct**, публикуется байт-в-байт из `templates/routing/incy.json`. Импорт и фактическая маршрутизация в INCY требуют проверки на клиенте. При обновлении файл заменяется версией из репозитория.
...
## Обновление 0.1.2: независимые клиенты
...
## 0.2.0 — AmneziaWG и имена входящих
