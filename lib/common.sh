...
backup_begin() {
  ...
  for path in /etc/nginx /etc/x-ui /usr/local/x-ui /etc/systemd/system/x-ui.service /var/www/single443 /etc/single443/firewall.nft /etc/single443/firewall-expected.txt /etc/systemd/system/single443-firewall.service; do
    if [[ -e $path ]]; then
